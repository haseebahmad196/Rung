...
export async function POST(request: Request) {
  const body = (await request.json()) as SelectTrumpBody;
  ...
  try {
    ...
    manager.selectTrump(tableId, playerId, cardId);
    ...
    return NextResponse.json(
      {
        error: error instanceof Error ? error.message : "Failed to select trump.",
      },
      { status: 400 }
    );
  }
